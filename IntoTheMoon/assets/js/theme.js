...
var theme = {
    ...
    init: function () {
        ...
        // Preloader - MUST be registered early to ensure it always runs
        theme.preloader();
        ...
        // Countdown (wrapped for safety)
        try { theme.countdown(); } catch (e) { console.warn('countdown:', e); }

        // Room Slide
        theme.roomSlide();

        // Plyr Player (wrapped for safety)
        try { theme.plyrPlayer(); } catch (e) { console.warn('Plyr:', e); }

        // Animation (wrapped for safety)
        try { theme.scrollCue(); } catch (e) { console.warn('scrollCue:', e); }

        // Bootstrap dSelect (wrapped for safety)
        try { theme.dSelect(); } catch (e) { console.warn('dSelect:', e); }
        ...
        // Hotel datepicker (wrapped for safety)
        try { theme.datePicker(); } catch (e) { console.warn('datePicker:', e); }

        // Room slide (wrapped for safety)
        try { theme.swiperSlider(); } catch (e) { console.warn('Swiper:', e); }

        // Glightbox (wrapped for safety)
        try { theme.gLightbox(); } catch (e) { console.warn('GLightbox:', e); }

        // Bootstrap validation
        theme.bsValidation();

        // Highlight validation (wrapped for safety)
        try { theme.highlight(); } catch (e) { console.warn('highlight:', e); }

        // Code Snippet (wrapped for safety)
        try { theme.codeSnippet(); } catch (e) { console.warn('codeSnippet:', e); }
    ...
    preloader: () => {
        ...
        if (preloader) {
            ...
            // Function that triggers removal with a small delay for smooth transition
            const hidePreloader = () => {
                setTimeout(() => {
            ...
            // Primary: hide preloader as soon as the DOM is ready
            document.addEventListener('DOMContentLoaded', hidePreloader);

            // Fallback: force hide after 5 seconds regardless of load state
